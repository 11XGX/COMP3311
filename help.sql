create type stu_res as (student text, score numeric(5,2));
...
create function next_state(p IntPair, n integer) returns IntPair
as $$
begin
    if (p.x is null) then
        p.x := n;
...
create function second(p IntPair) returns integer
as $$
begin
    return p.y;
end;
